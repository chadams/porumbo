...
(function (window, initComponents, undefined) {
  ...
  Crafty.fn = Crafty.prototype = {
    ...
    trigger: function (event, data_n) {
      /**
       *
       * *Note: modified to accept more than one argument for data
       * This is necessary because SaveData Event passes (data, prepare)
       * but prepare was always undefined.
       * http://craftyjs.com/api/Storage.html#SaveData-event
       */
      // (To learn how the handlers object works, see inline comment at Crafty.bind)
        ...
        return {
          open: function (gameName_n) {
            ...
            if (arguments.length == 1) {
              ...
              stores.shift();
              // can pass version as last argument
              var _v = stores.pop();
              if(isNumber(_v)){
                _version = _v;
              }else{
            ...
            if (db == null) {
              var request = indexedDB.open(gameName, _version);
